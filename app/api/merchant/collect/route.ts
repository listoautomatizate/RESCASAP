...
export async function POST(request: Request) {
  ...
  const reservation = await env.DB.prepare(`SELECT r.id, r.status, pt.status AS online_payment_status FROM reservations r
    ...
    WHERE r.pickup_code = ? AND b.owner_id = ? LIMIT 1`).bind(code, authUser.userId).first<{
      id: string; status: string; online_payment_status: string | null;
    }>();
  ...
  if (reservation.online_payment_status && reservation.online_payment_status !== 'paid') {
